...
async def check_user_last_interaction(user_data):
    # check if 15min passed, if true, clear the active flow from the DB and restart
    if user_data.get('active_flow') and await remove_expired_active_flow(user_data):
        # clean the user from local active_users map
        active_users_map.pop(user_data["_id"], None)
...
async def trigger_restart_flow(update: Update, context: CallbackContext) -> None:
    ...
    await start_flow(user_id, RESTART_FLOW.get_id(), question.get_id())
